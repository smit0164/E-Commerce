...
<div class="bg-white p-8 rounded-xl shadow-lg max-w-7xl mx-auto">
    <h2 class="text-3xl font-bold text-gray-900 mb-6">Edit Static Page</h2>

    <form action="{{ route('admin.static_pages.update', $staticPage->slug) }}" method="POST" id="staticEditPageForm">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="title" id="title" class="w-full px-3 py-2 border rounded @error('title') border-red-500 @enderror" value="{{ old('title', $staticPage->title) }}" placeholder="Enter your title">
                    @error('title')
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" class="w-full px-3 py-2 border rounded @error('slug') border-red-500 @enderror" value="{{ old('slug', $staticPage->slug) }}" placeholder="Slug will appear" readonly>
                    @error('slug')
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea name="content" id="summernote" class="w-full @error('content') border-red-500 @enderror" required>{{ old('content', $staticPage->content) }}</textarea>
                @error('content')
        ...
        <div class="mt-6">
            <button type="submit" class="bg-indigo-600 text-white hover:bg-indigo-700 px-4 py-2 rounded">Update</button>
