            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="file" 
                           ...
                           <div id="image-preview" class="mb-2 flex flex-wrap gap-2"></div>
                    @error('image')
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror
    ...
    $('#product-image').on('change', function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(event) {
                ...
                $('#current-image').addClass('hidden'); // Hide current image
            };
            reader.readAsDataURL(file);
        } else {
            $('#current-image').removeClass('hidden'); // Show current image if no new file
        }
    });
    ...
    $('#edit-product-form').validate({
        rules: {
            name: {
                ...
                minlength: 3,
                remote: {
                    url: '{{ route('admin.products.check-unique-for-edit') }}',
                    type: 'POST',
                    data: {
                        name: function() {
