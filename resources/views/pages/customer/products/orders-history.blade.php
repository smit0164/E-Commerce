    ...
    <script>
        $(document).ready(function(){
            ...
            function updateModal(order) {
                const shippingText = order.shipping_address ? `${order.shipping_address.full_name || ''}<br>` +
                    `${order.shipping_address.address_line1 || 'N/A'}, ` +
                    `${order.shipping_address.address_line2 ? order.shipping_address.address_line2 + ', ' : ''}` +
                    `${order.shipping_address.city || ''} ${order.shipping_address.postal_code || ''}<br>` +
                    `${order.shipping_address.state || ''}, ${order.shipping_address.country || ''}` : 'Not provided';
                
                $('#shipping-address').html(shippingText);

                const billingText = order.billing_address ? `${order.billing_address.full_name || ''}<br>` +
                    ...
                    `${order.billing_address.state || ''}, ${order.billing_address.country || ''}` : 'Not provided';
